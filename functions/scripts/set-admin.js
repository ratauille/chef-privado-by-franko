...
if (fs.existsSync(serviceAccountPath)) {
  ...
  console.log('🔑 Inicializado Firebase Admin con serviceAccountKey.json local.');
} else {
  let token = '';
  try {
  ...
  if (token) {
    ...
    console.log('🔑 Inicializado Firebase Admin con token de gcloud.');
  } else {
    admin.initializeApp({
      projectId: 'chef-privado',
    });
    console.log('ℹ️ Inicializado Firebase Admin con credenciales por defecto (ADC).');
  }
}
